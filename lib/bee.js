...
Bee.prototype._duplicate = function (job) {
    ...
    return Promise.try(typeof self.spec.hash === 'function' ? self.spec.hash : self._defaultHashFunc, [job].concat(job.data))
        .timeout(self.spec.timeout, 'Hash method timed out after ' + self.spec.timeout + ' ms')
        .then(function (hash) {
...
Bee.prototype._do = function (job) {
    ...
    return Promise.try(self.spec.worker, [job].concat(job.data))
        .timeout(self.spec.timeout, 'Worker method timed out after ' + self.spec.timeout + ' ms')
        // process result
