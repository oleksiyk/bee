local getJob = function(key_jobs)
    local duplicate_job = false

    local jid, parent, data, queue, worker, hash, status, options, submitted, retries, tags, result, duplicate, failed_reason, history = unpack(redis.call('hmget', key_jobs,
        'jid', 'parent', 'data', 'queue', 'worker', 'hash', 'status', 'options', 'submitted', 'retries', 'tags', 'result', 'duplicate', 'failed_reason', 'history'))
