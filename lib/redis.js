...
var Redis = function (options, hiveId) {
    ...
    _.each(['evalsha', 'script', 'publish', 'zrank', 'set', 'hgetall', 'sunion', 'sinter'], function (command) {
...
Redis.prototype.job_put = function (job) {
    var keys = [
        'bee:ss:queue:' + job.queue,
        'bee:h:jobs:' + job.jid,
        'bee:ss:delayed:' + job.queue
    ];

    if (job.parent){
        keys.push('bee:h:jobs:' + job.parent)
        keys.push('bee:s:' + job.parent + ':children')
    }
